...
public class FingerprintAPI {
	 ...
	 *         chaotic fragmentation characteristics Confirmation code=07H
	 *         Represents the fingerprint image is normal, but too few feature
	 *         points fragmentation characteristics and health Confirmation
	 *         code=15H Means that there is no valid image buffer born
	 *         fragmentation original image image Confirmation code=ffH
	...
	public synchronized int PSGenChar(int bufferId) {
		...
		sendCommand(command);
		int length = chatService.read(buffer, 8000, 200);
	 ...
	 *         确认码=ffH 表示无响应 Function Description: Merge features generate a
	 ...
	 *         确认码=18H表示写FLASH出错 确认码=ffH 表示无响应 Function Description: The
...
//		Log.i("whw", "storechar  command=" + DataUtils.toHexString(command));
		sendCommand(command);
		int length = chatService.read(buffer, 4000, 200);
	 ...
	 *         确认码=0BH表示PageID超出指纹库范围 确认码=ffH 表示无响应 Function Description:
	 ...
	 *         bufferId:1h or 2h pageId：0~1010 Input parameters: BufferID
	 ...
	 *         表示无响应 Function Description:In CharBuffer1 or CharBuffer2 the
	...
	public synchronized Result PSSearch(int bufferId, int startPageId,
		...
		byte[] command = { (byte) 0xef, (byte) 0x01, (byte) 0xff, (byte) 0xff,
				...
				(byte) checkSumArray[0], (byte) checkSumArray[1] };
//		Log.i("whw", "PSSearch  command=" + DataUtils.toHexString(command));
//		Log.i("whw", "startPageId=" + startPageId + "    pageNum=" + pageNum);
		sendCommand(command);
		int length = chatService.read(buffer, 4000, 200);
	 ...
	 *
	 * @return true：指纹匹配成功 false：比对失败 Function Description: Exact Match
	 ...
	 *
	 * @return 确认码=00H 表示搜索到； 确认码=01H 表示收包有错； 确认码=09H 表示没搜索到；此时页码与得分为0 确认码=ffH
	 *         表示无响应 Function Description: Automatic fingerprint in the
	 ...
	 *         And the target template is incomplete collection characteristic
	 *         feature places a blank area to update the target template. Return
	 ...
	 *         number of the deleted template.
	 * @param pageIDStart
	 * @param delNum
	...
	/**
	 * 将特征缓冲区中的特征文件上传给上位机 Function Description: The characteristic features of
	 * the buffer file uploads to the host computer (the default feature buffer
	 * charbuffer1)
	 *
	 * @return byte[]：长度为512字节成功 否则失败 null:上传特征文件失败 byte[]：Length success is 512
	 ...
	 *         确认码=ffH 表示无响应 Function Description: download the signature file
	...
	public synchronized int PSCalibration() {
		...
		sendCommand(command);
		int length = chatService.read(buffer, 15000, 100);
	...
	/**
	 * 将图像缓冲区的数据上传给上位机 Function Description: Upload the image buffer data to the
	 * host computer;
	 *
		...
		while (!isExit) {
			if (isStart) {
				...
				if (startLength == size || dataLength == size) {
					...
					// String testHexStr = DataUtils.toHexString(test);
					// int testHexStrLength = testHexStr.length()/2;
					// Log.i("whw", "testHexStr length="+testHexStr.length());
					// Log.i("whw",
					// "----------upimage fail size hex1="+testHexStr.substring(0,
					// testHexStrLength));
					// Log.i("whw",
					// "----------upimage fail size hex2="+testHexStr.substring(testHexStrLength));
					// sendCommand(nextCommand);
					// //test
					break;
				}
			...
			size = chatService.readImage(buffer, 8000, requestLength);
		}
...
//		int dataPacektLength = 0x12;
		...
		byte[] endPrefix = { (byte) 0xef, (byte) 0x01, (byte) 0xff,
				...
				(byte) 0x00, (byte) dataPacektLength };
		byte[] command = new byte[dataPrefix.length + dataPacektLength];
		int length = data.length / (dataPacektLength - 2);
		for (int i = 0; i < length; i++) {
			...
			System.arraycopy(data, i * (dataPacektLength - 2), command,
					dataPrefix.length, (dataPacektLength - 2));
			short sum = 0;
